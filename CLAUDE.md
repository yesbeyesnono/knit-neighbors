...
- **푸시는 묶어서(2026-09-20 대표 확정)**: main 푸시 = KOAP TestFlight 빌드 1회. Apple은 앱당 하루 업로드 횟수를 제한한다(2026-09-19 하루 십수 회 푸시 → `iris-code 90382 Upload limit reached`로 업로드 거절, 24시간 뒤 해제). 그래서 수정마다 푸시하지 말고 **커밋만 해 두고**, 보고 끝에 "푸시는 아직 안 했습니다 — 계속 말씀하시거나 '올려줘' 하세요"라고 알린다. 대표가 **"올려줘"**라고 하거나 작업 묶음이 끝났을 때 한 번만 푸시. 하루 푸시는 몇 번 이내로. **주의: HEAD 커밋 메시지의 `[skip ci]`는 Codemagic뿐 아니라 GitHub Pages 배포(Actions)도 건너뛴다** — 콘솔·웹만 올릴 때 `[skip ci]`로 푸시했으면 바로 `gh api -X POST repos/yesbeyesnono/knit-neighbors/pages/builds`로 Pages 빌드를 수동 요청하고 `curl`로 반영을 확인할 것(2026-09-26 "안됨" 사례).
...
- **기법 고르기(2026-09-26 대표 지시)**: 작품 인증·모임 '배울 기법'·모임 글의 '배우기 ›'는 전부 내 뜨개 단계와 같은 기법 맵(`techBoard(craft, cls, onClick, ver)` 공용, `techPickBoard`)으로 그린다 — 고른 기법=진한 칸, 그 선행 기법=점선(먼저 알아야 해요). 모임 배울 기법은 여러 개(`meetups.technique_ids`, 첫 번째는 `technique_id`에도). 모임 폼 칩은 종목(대바늘/코바늘)이 맨 앞
...
- **공지·알림 보내기(2026-09-26 정밀 타겟팅)**: 콘솔 › 공지·알림 — 조건 스펙 `BC_FIELDS`(지역·종목/실력·활동·역할/상태·관심 + 제외·상한/무작위)가 서버 `_bc_ids` 키와 1:1. 미리보기(인원+샘플) → 나에게 테스트 → 발송/예약(매주·매월 반복, 크론 5분) · 채널 🔔 알림/지기 채팅 · 딥링크(`notifications.link` → 앱 `openLink`) · 저장된 그룹 · 기록에 열람/클릭 수 · jigi `segment`(말로 정하기 → 조건 채움)·`bc_polish`. 상세 supabase_schema.sql 51
- 콘솔 메뉴: 오늘(기본 화면, `#today:<id>`로 건별 패널 — 맨 위 '기다리는 신청·문의' 묶음: 가게·작가·제보·문의·이벤트 대기 건수, 브리핑에도 같은 줄) · 규칙 · 기록·되돌리기 · 기법 후보 · 욕설 사전. AI 없이도 1단계 감지·버튼 처리·'목록/규칙/자동' 명령은 동작
...
- **지기 AI 1차 응대(2026-09-26 대표 확정)**: 마이 › 설정 › 지기에게 말하기의 회원 메시지에 `jigi` route `support`가 몇 초 안에 '지기 AI' 이름으로 응답 — 역할은 ① 불편을 정확히 파악(질문 최대 2~3번) ② **대표가 승인한 FAQ(support_kb)만 직접 답변** ③ 그 외는 해결·약속 없이 대표에게 전달("보통 하루 안에"). 티켓(support_tickets) → 텔레그램(알림에 **답장**하면 회원에게 전달, AI가 존댓말로 다듬은 초안/원문 선택) + 콘솔 › 지기 문의함(티켓·AI 요약·답변·다듬기·통계). 답변 다음 날 "해결되셨나요?" 자동 확인. 학습은 매주 월요일 AI가 FAQ·용어·확인 항목 **후보만 제안** → 콘솔 › 지식 창고에서 승인한 것만 사용. 원칙은 지기 AI와 동일(회원 글=자료, 개인정보 요구 금지, 위험 표현 긴급). 상세는 supabase_schema.sql 46
...
- **GRANT 규칙(Supabase 2026-10-30 변경, 대표 전달)**: 새 public 테이블은 자동 grant 가 안 된다. 테이블을 만드는 **같은 마이그레이션 안에** 반드시 `revoke all on public.<t> from anon, authenticated;` + 필요한 만큼만 `grant select[, insert, update, delete] on public.<t> to authenticated;` + `grant all on public.<t> to service_role;`(Edge Function 용)를 쓴다. anon 은 기본적으로 주지 않는다. RLS 만 켜고 grant 를 빼먹으면 앱에서 permission denied. 2026-09-24 점검: 기존 테이블은 전부 grant 있음(api_rate·jigi_chat·jigi_config·mod_pending 은 service_role 전용이 맞음)
- **새 테이블 GRANT 필수(Supabase 공지, 2026-10-30부터)**: public 스키마에 새 테이블을 만들면 Data API 권한이 자동으로 붙지 않는다. 테이블을 만드는 **같은 마이그레이션 안에** 반드시 `grant select on public.<t> to anon;` `grant select, insert, update, delete on public.<t> to authenticated;` `grant select, insert, update, delete on public.<t> to service_role;`를 넣을 것(비로그인이 볼 필요 없는 테이블은 anon 생략, RLS는 별도로 켬). 기존 테이블은 영향 없음. 빠뜨리면 API가 permission denied.
...
## 가게·도안·작가 (2026-09-18)
- 가게 가입(2026-09-26 개편): 설정 › 우리 가게 등록 신청(상호·종류·주소 검색→좌표·**이메일 필수**·연락처·사업자번호·등록증 사진·인스타/스토어/홈페이지 링크) → 트리거로 `jigi` route `shop_claim`이 사업자등록증을 AI로 읽어 **사업자번호·상호·주소 셋 다 일치하면 자동 승인**(좌표 있으면 바로 공개, 서류 삭제, 텔레그램 통보) / 아니면 콘솔 오늘(mod_items kind shop_claim) + 텔레그램 [승인][반려]. 승인 뒤 오너는 **마이 › 설정 › 내 가게 관리**(`openShopEdit`: 로고 버킷 shops·소개·영업시간·전화·링크·주소 검색)로 직접 수정, 공개/오너 지정만 콘솔. 상세 supabase_schema.sql 49
- **클래스 열기**(가게 오너, `openClassForm` → `v-classform`, shop_posts kind class + extra v1): 양식은 솜씨당·탈잉·프립·네이버 예약 조사 결과로 확정 — 대표 사진·클래스명·한 줄 소개·종목·난이도(N단계)·배울 기법(기법 맵)·완성작·원데이/정규(회차·주기)·일시·소요·모집 마감·최소/최대 인원·수강료·재료비(포함/별도 금액)·제공/준비물·장소·소개/추천/커리큘럼/강사·신청 방법(링크/전화/인스타/직접)·취소환불 규정 프리셋·유의사항. 가게 화면 `classBox()` 카드에 '최소 인원 미달 시 전액 환불'·'결제는 가게가 직접' 문구 자동. 앱 내 결제 없음
...
## 1기 서포터즈 (2026-09-26, 지시서 knitup/docs/supporters_spec.md)
